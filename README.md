...
- **零额外依赖** — 不需要 openssl、不需要 busybox，仅使用 Android 自带的 toybox 工具
...
teeforge --rootdetect     # 检测 root 方式并输出
...
| keybox 解密 | 四层编码：XOR + 10×base64 + hex + ROT13 |
...
安装后编辑 `/data/adb/teeforge/config.conf`：
