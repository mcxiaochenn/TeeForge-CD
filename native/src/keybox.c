...
static int keybox_decrypt(const char *enc_data, size_t enc_len,
    ...
    /*
     * 上游 Megatron 解密流程（参考 upstream reference）：
     * Upstream Megatron decryption (reference: upstream reference):
     * 1. 多层 base64 解码 [多层 base64 decode]
     * 2. hex 解码 [hex decode]
     * 3. ROT13 解码 [ROT13 decode]
     */

    /* 步骤 4：多层 base64 解码 Step 4: 多层 base64 decode (纯 C，无临时文件) */
